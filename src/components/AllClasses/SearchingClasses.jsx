...
const SearchingClasses = ({ totalClasses = 0 }) => {
  ...
  const categories = ["All Categories", "Yoga", "Cardio", "Weights", "Combat", "Dance"];
    ...
    <div className="flex flex-col items-center gap-6 mb-8 w-full max-w-3xl mx-auto">
      ...
      <div className="flex flex-wrap justify-center gap-3">
        {categories.map((cat) => (
          <button
            ...
            className={`px-5 py-2.5 rounded-full font-['Inter'] text-sm font-bold tracking-wide transition-all duration-300 cursor-pointer ${
              category === cat
                ? "bg-active text-btn-text shadow-md border border-active"
                : "bg-brand-900/10 dark:bg-brand-800 text-foreground/80 dark:text-brand-300 border border-brand-500/20 hover:border-brand-500 hover:text-foreground"
            }`}
          >
      ...
      <div className="flex justify-between items-center w-full mt-4 text-sm text-[#535C91] dark:text-[#9290C3] border-b border-brand-500/10 pb-4">
        <div className="flex items-center gap-2">
          ...
          <span>
            Showing <span className="font-bold text-foreground">{totalClasses > 0 ? "1" : "0"}-{totalClasses}</span> of <span className="font-bold text-foreground">{totalClasses}</span> classes
          </span>
        </div>
